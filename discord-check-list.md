...
- [ ] `twilight` aggregate crateが必要か確認する。
- [ ] `twilight-gateway` が必要か確認する。
- [ ] `serde_json` が必要か確認する。
- [ ] その他の未使用依存を確認する。
- [ ] 不要な依存を削除する。
- [ ] `Cargo.lock` を更新する。
...
- [ ] `Cargo.toml` に `rust-version` を追加するか検討する。
- [ ] `rust-toolchain.toml` を追加するか検討する。
- [ ] `edition = "2024"` を維持する場合、READMEに必要Rustバージョンを書く。

## 6.3 ログ・エラー方針

- [ ] `anyhow` をアプリ境界で使う方針は維持してよい。
- [ ] 内部エラー型に `thiserror` が必要か検討する。
- [ ] `log/env_logger` を維持するか、`tracing` へ寄せるか判断する。
- [ ] この段階での `tracing` 移行は必須にしない。

## 6.4 受け入れ条件

- [ ] 未使用依存が整理されている。
- [ ] `cargo clippy --all-targets --all-features -- -D warnings` が通る。
- [ ] Rustバージョン要件がREADMEか設定ファイルに明示されている。
