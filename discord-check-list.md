...
- [x] titleなしitemを扱える。
- [x] linkなしitemを扱える。
- [x] descriptionなしitemを扱える。
- [x] pubDate不正itemを扱える。
- [x] Atom対応するならAtomもテストする。
  - 現行方針はRSSのみのため、Atomテストは追加しない。
...
- [x] 長文descriptionを短縮する。
- [x] titleを短縮する。
- [x] linkを残す。
- [x] HTML混入を扱う。
- [x] メンション混入を扱う。
- [x] 空title/descriptionを扱う。

## 7.6 受け入れ条件

- [x] `cargo test` が通る。
- [x] ネットワークを使うテストはmock化されている。
  - Phase 7で追加したテストは文字列RSS、状態ファイル、設定値、フォーマット関数を対象にし、ネットワークを使わない。
- [x] 主要ロジックがユニットテストで保護されている。
...
- [x] clone URLを実リポジトリに修正する。
- [x] ディレクトリ名を実リポジトリに合わせる。
- [x] プロジェクト名を統一する。
- [x] 実装していない機能説明を削る。
- [x] RSS/Atom対応状況を正確に書く。

## 9.2 セットアップ手順

- [x] 必要Rustバージョンを書く。
- [x] `.env_example` から `.env` を作る手順を書く。
- [x] `config/default.example.toml` からローカル設定を作る手順を書く。
- [x] Discord Bot Tokenの作り方を書く。
- [x] 必要権限を書く。
- [x] `cargo run` での起動手順を書く。

## 9.3 初回起動・重複管理

- [x] 初回起動時に既存記事を投稿するのか書く。
- [x] 初回起動時に既存記事をスキップするのか書く。
- [x] 状態ファイルの場所を書く。
- [x] 状態ファイルを削除した場合の挙動を書く。

## 9.4 常時運用

- [x] systemd運用例を書くか検討する。
  - 具体的なunit追加はPhase 10対象とし、Phase 9では方針説明に留める。
- [x] Docker運用例を書くか検討する。
  - 具体的なDockerfile追加はPhase 10対象とし、Phase 9では方針説明に留める。
- [x] `RUST_LOG=info` の使い方を書く。
- [x] ログ確認方法を書く。
...
- [x] tokenをcommitしない。
- [x] webhook URLを公開しない。
- [x] RSS本文由来のmention事故に注意する。
- [x] `.env` は `.gitignore` に含める。

## 9.7 受け入れ条件

- [x] READMEだけ読めばセットアップできる。
- [x] READMEの説明と実装が一致している。
- [x] clone URLが正しい。
- [x] サンプル設定が安全である。
